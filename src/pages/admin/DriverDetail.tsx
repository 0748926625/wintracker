...
export default function AdminDriverDetail() {
  ...
  const livres = packages.filter((p) => p.status === 'LIVRE' && pf.inRange(effectiveDate(p)))
  const retours = packages.filter((p) => p.status === 'RETOUR' && pf.inRange(effectiveDate(p)))
  const echecs = packages.filter((p) => p.status === 'ECHEC' && pf.inRange(effectiveDate(p)))
